...
DIFF_BIT = 43
KEY_SIZE = 80
R=BooleanPolynomialRing(80+ 160 + 40 + 61,['k%d'%(i) for i in range (80)]+['v%d'%(i) for i in range (160)]+['n%d'%(i) for i in range (40)]+['l%d'%(i) for i in range (61)])
R.inject_variables()

def gen_params():
    #setting up the LFSR
    L1 = [ZZ.random_element(2) for i in range(LFSR_SIZE)]
    L2 = L1[:]
    L2[43]+= 1
...
def gen_forward_stream(params):
    ...
    for  ind in range(KEY_SIZE):
        ...
        _L1[LFSR_SIZE -1] = L_update_1
        _L2[LFSR_SIZE -1] = L_update_2
...
def gen_backward_stream(params):
    ...
    while tt >= 0:
        ...
        L_update_1=_L1[LFSR_SIZE -1]
        N_update_1=_N1[NFSR_SIZE -1]

        L_update_2=_L2[LFSR_SIZE -1]
...
def generate_eqs(params , guesses):
    ...
    for  ind in range(KEY_SIZE):
        ...
        L_update= R(L[0]+L[14]+L[20]+L[34]+L[43]+L[54])
        ct = ind % 80
        c4 = (ct//16)%2
        keybit = R(K[ind])

        N_update = R(L[0]+ c4+ keybit+ N[0]+N[13]+N[19]+N[35]+N[39]+ N[2]*N[25] + N[3]*N[5] +N[7]*N[8] + N[14]*N[21] + N[16]*N[18]+ N[22]*N[24] + N[26]*N[32] + N[33]*N[36]*N[37]*N[38] + N[10]*N[11]*N[12]+ N[27]*N[30]*N[31])
        ...
        #change the last bit of NFSR and LFSR
        L[LFSR_SIZE -1] = L_update

        N[NFSR_SIZE -1] = R('v%d'%ind)
        EQ_F.append(R('v%d'%ind) - N_update)
...
def solver():
    ...
    guesses = [i for i in range(20)]

    #create the equations
    EQ = generate_eqs(params , guesses)

    #construct an ideal of the equations
    I = Ideal(EQ).gens()

    #the sat solver
    import sage.sat.boolean_polynomials
    C= sage.sat.boolean_polynomials.solve(I)
    print C
